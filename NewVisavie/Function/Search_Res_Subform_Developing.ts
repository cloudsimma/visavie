try 
{
	...
	{
		query = "Select Parent_Id.Residence_number ,Parent_Id.Name ,Parent_Id.Region,Parent_Id.id  FROM  Subform_21  WHERE ";
		//query = "";
		end_srting = " ";
		open_bracket_final = "";
		count11 = 1;
		...
		group_by = " Group by Parent_Id.Residence_number ,Parent_Id.Name ,Parent_Id.Region,Parent_Id.id";
		if(PageNo == 1)
		{
			query = query + end_srting + group_by + "";
			QueryCount = query;
		}
		else
		{
			...
			query = query + end_srting + group_by + searchFilter + "";
			QueryCount = query + end_srting;
		}
	}
	//info query;
	//query = "Select Parent_Id.Residence_number ,Parent_Id.Name ,Parent_Id.Region,Parent_Id.id FROM Subform_21 WHERE (Parent_Id.Status = 'Active') and ((Minimum_rate_1 >= 1000 and Minimum_rate_1 <=2000)) Group by Parent_Id.Residence_number ,Parent_Id.Name ,Parent_Id.Region,Parent_Id.id";
...
catch (e)
{
	...
	dataMap.put("Process_Description","Search Res Subform developing");
