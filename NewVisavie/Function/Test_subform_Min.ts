...
try 
{
	...
	if(input.TypeRes.size() > 0)
	{
		...
		strcretria = "";
		for each  ele in TypeRes.toList(";")
		{
			info ele;
			strcretria = "(" + strcretria + "Region like '%" + ele + "%')" + " or ";
		}
		strcretria = strcretria.removeLastOccurence("or").trim();
		query_list.add(strcretria);
	}
	...
	if(input.Status.size() > 0)
	{
		...
		Status_query = openBracket + Status_query + "))";
	...
	// 	}
	// }
	// 	((((((Parent_Id.Status = 'Active - Entente particulière/Active - Special agreement') or (Parent_Id.Status = 'Fermée par la direction/Closed by management')) or (Parent_Id.Status = 'Suspendue temporairement - Autres raisons/Temporarily suspended- other reasons')) or (Parent_Id.Status = 'Active')) or (Parent_Id.Status = 'Fermée/Closed')))
	// 	query1 = "Select Residence_number,Name,Nom_du_groupe,Type_of_residence,Status,Region,City FROM Residence WHERE (((((Status = 'Active - Entente particulière/Active - Special agreement') or (Status = 'Fermée par la direction/Closed by management')) or (Status = 'Suspendue temporairement - Autres raisons/Temporarily suspended- other reasons')) or (Status = 'Active')))";
...
catch (e)
{
	...
	dataMap.put("Process_Description"," Search Residence Widget Subform");
