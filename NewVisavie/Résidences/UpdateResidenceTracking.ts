try 
{
	...
	if(getresidence.get("id") != null && getresidence.get("Tracking_Id") != null)
	{
		dataMap = Map();
		...
		for each  residenceSubform in subformInfo
		{
			...
			if(residenceSubform.get("Contact") != null)
			{
				...
				{
					dataMap.put("Send_client_profile_Email_ID_s","");
				}
				...
				{
					dataMap.put("Send_Invoice_Email_ID_s","");
				}
		...
		updateResiTracking = zoho.crm.updateRecord("Tracking",getresidence.get("Tracking_Id").toNumber(),dataMap);
	}
	else
	{
		dataMap = Map();
		...
		for each  residenceSubform in subformInfo
		{
			...
			{
				res_map.put("Contact_Name",residenceSubform.get("Contact").get("id"));
				if(residenceSubform.get("Send_profile_confirmation") == true || residenceSubform.get("Billing_Confirmation") == true || residenceSubform.get("Send_profile_confirmation") == true && residenceSubform.get("Billing_Confirmation") == true)
				{
					dataMap.put("Send_client_profile_Email_ID_s",residenceSubform.get("Email"));
				}
				resList.add(res_map);
...
catch (e)
{
	...
	dataMap.put("Process_Description","CRM: Update residence Tracking Record");
