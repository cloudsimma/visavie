		...
		else
		{
			...
			else if(getLeads.get("Test") == "2 démarches avec 2 prospects/2 deals with 2 leads")
			{
				...
				if(getLeads.get("Prospect_First_Name2") != "" && getLeads.get("Prospect_Last_Name2") != "")
				{
					...
					primaryContactMap.put("Work_Phone_Extension",getLeads.get("Work_phone_Extension_2"));
					primaryContactMap.put("Layout","4846491000000339001");
					primaryContactMap.put("Contact_Type","Client");
					Contact_Counselor = getLeads.get("Advisors");
					if(Contact_Counselor != null)
					...
					if(createPrimaryContact_2.get("id") != null)
					{
						...
						if(Client_Info_2.get("id") != null)
						{
							Update_contact = Map();
							Update_contact.put("Client_Number",Client_Info_2.get("Num_ro_Client"));
							homeDealMap.put("Client_1_number",Client_Info_2.get("Num_ro_Client"));
							update_Client_c2 = zoho.crm.updateRecord("Contacts",Client_contact_ID,Update_contact);
						}
					}
					homeDealMap.put("Contact",createPrimaryContact_2.get("id"));
					homeDealMap.put("Client_1_Provinces",getLeads.get("Prospect_Province2"));
					if(getLeads.get("Do_you_like_to_enter_prospect_2_profile_details") == "Oui/Yes")
					{
						/* Lead  Profile -2*/
						homeDealMap.put("Type_s_of_client_Type_s_de_client",getLeads.get("Profile_Type_s_de_client_Type_s_of_client"));
					...
					if(createPrimaryContact_2.get("id") != null)
					{
						...
						primary_map.put("Work_Phone_Extension",getLeads.get("Work_phone_Extension_2"));
