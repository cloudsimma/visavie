...
<body>
<%
	response = zoho.crm.getRecordById("Deals",DealCRMId.toLong(),Map(),"zoho_one");
	if(response.get("id").toList().size() > 0)
	{
		if(response.get("Contact") != null)
		{
			%>
...
<%
			if(response.get("Counselor_Conseiller") != null)
			{
				get_counselor = Advisor[CRM_AdvisorID == response.get("Counselor_Conseiller").get("id")];
...
<%
			...
			get_contact = zoho.crm.getRecordById("Contacts",response.get("Contact").get("id").toLong(),Map(),"zoho_one");
...
<%
			...
			get_contact = zoho.crm.getRecordById("Contacts",response.get("Contacts").get("id").toLong(),Map(),"zoho_one");
			if(get_contact.get("Full_Name") != null || get_contact.get("Date_of_Birth") != null || response.get("Contact_persons").size() > 0 || get_contact.get("Ville_City") != null)
			{
				%>
<div class="bgclr">CLIENT(E) 1/CLIENT 1</div>
