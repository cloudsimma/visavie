...
try 
{
	...
	if(getLeads.get("id") != null)
	{
		...
		if(getLeads.get("Deal_Conversion") == "Créer 1 démarche avec 1 prospect/Create 1 deal using 1 prospect")
		{
			...
			resp = "La démarche soin a été créée avec succès/Home care Deal has been created successfully";
			openUrl("https://crm.zoho.com/crm/org746753262/tab/CustomModule5/" + createHouseCareDeal_1.get("id"),"new window");
			...
			if(createHouseCareDeal_1.get("id") != null)
			{
				...
				if(getdeal_info.get("id") != null)
				{
					...
					icreatorDealResp = zoho.creator.createRecord(ownerName,appName,formName,Creator_housingDealMap,Map(),"zoho_mail");
				}
			}
			/* delete lead*/
			deleteRecordMap = Map();
		...
		else if(getLeads.get("Deal_Conversion") == "Créer 1 démarche avec 2 prospects/Create 1 deal using 2 prospects")
		{
		...
		else if(getLeads.get("Deal_Conversion") == "Créer 2 démarches avec 2 prospects/Create 2 deals using 2 prospects")
		{
			...
			if(getLeads.get("Prospect_First_Name2") != "" && getLeads.get("Prospect_Last_Name2") != "")
			{
				...
				if(createPrimaryContact_res.get("id") != null)
				{
					...
					Update_contact_C2 = Map();
					Update_contact_C2.put("Client_Number",get_Client_Info.get("Num_ro_Client"));
					homeDealMap.put("Client_2_number",get_Client_Info.get("Num_ro_Client"));
					update_Client_contact = zoho.crm.updateRecord("Contacts",Client_2_contact_ID,Update_contact_C2);
				}
				homeDealMap.put("Client_2",createPrimaryContact_res.get("id"));
				if(getLeads.get("Do_you_like_to_enter_prospect_2_profile_details") == "Oui/Yes")
				{
					/* Lead  Profile -2*/
					...
					homeDealMap.put("Profile_2_Service_s_sought",getLeads.get("Profile_Type_s_de_service_s_Type_s_of_service_s"));
			...
			resp = " 2 La démarche soin a été créée avec succès/2 Home care Deals has been created successfully";
			openUrl("https://crm.zoho.com/crm/org746753262/tab/CustomModule5/","new window");
			...
			if(createHouseCareDeal_2.get("id") != null)
			{
				...
				if(deal_info.get("id") != null)
				{
					...
					// Profile 2 client
					Contact_info_2 = deal_info.get("Client_2");
					if(Contact_info_2 != null)
					{
						housingDealMap.put("Contact2_CRM_ID",Contact_info_2.get("id"));
					}
					housingDealMap.put("Profile_2_Type_s_of_client",deal_info.get("Profile_2_Type_s_of_client_Type_s_de_client"));
					housingDealMap.put("Profile_2_Desired_moving_date",deal_info.get("Profile_2_Desired_moving_date_Date_de_d_m_nagement"));
					housingDealMap.put("Profile_Service_s_sought",deal_info.get("Profile_2_Service_s_sought"));
					housingDealMap.put("Profile_2_Region_Region",deal_info.get("Profile_2_R_gion_Region"));
